...
MONGO_ADMIN_PASSWORD=${MONGO_ADMIN_PASSWORD:-"BAp3kTJGu9H-2+N"}

# Application Database User
MONGO_DATABASE=${MONGO_DATABASE:-"crowded"}
...
use $MONGO_DATABASE
db.createUser({user: '$MONGO_ADMIN_USERNAME', pwd: '$MONGO_ADMIN_PASSWORD', roles:[{role:'dbOwner', db:'$MONGO_DATABASE'},{role:'readWrite', db:'$MONGO_DATABASE'}]})
EOF
fi
