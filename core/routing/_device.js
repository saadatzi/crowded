...
router.post('/init', function (req, res) {
    logger.info('API: Device/init %j', {body: req.body});
    DeviceController.add(req.body.device)
        .then((deviceId) => {
            logger.info("*** dive added did: %s", deviceId);
        ...
        .catch(err => {
            logger.error("Device Add Catch err:", err)
            res.err(err)
        })
});
